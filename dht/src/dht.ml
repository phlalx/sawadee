...
let rec lookup_info_hash' t (nis:Node_info.t list) info_hash ~depth : ((Node_info.t list) * (Addr.t list)) Deferred.t =

  if depth = 0 then
    (nis, []) |> return
  else 
    let%bind l = Deferred.List.filter_map nis ~f:(lookup_info_hash t info_hash) ~how:`Parallel in 
    let f = function 
      | `Values x -> `Fst x 
      | `Nodes x -> `Snd x 
    in
    ...
    let%map better_nis, more_values = lookup_info_hash' t closest_nis info_hash ~depth:(depth - 1) in
