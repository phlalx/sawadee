...
let read_piece t p = 
  ...
  debug "Pers: read piece %d from disk" i; 
  let f seg : unit Deferred.t = 
    let { name; fd; off; len; off_in_file } = seg in
    (* debug !"Pers: reading piece %d from %{segment_to_string}" i seg;  *)
...
let read_from_pipe t =
  let read_piece p = 
    let i = Piece.get_index p in
    debug "Pers: read from pipe piece %d" i; 
    let f seg : unit Deferred.t = 
      let { name; fd; off; len; off_in_file } = seg in
      (* debug !"Pers: writing piece %d to %{segment_to_string}" i seg; *)
      let s = Piece.get_bigstring_content p in
      let pos = off % t.piece_length in
